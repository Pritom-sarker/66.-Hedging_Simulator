...
input             double MaxPairs=2;//Max Pairs
input             double MaxTradesPairs=5;//Max Trades in Pair
...
//+------------------------------------------------------------------+
bool TradeTime(int i)
  {
   bool result=false;

   datetime svt =UTCStartTime[i];
   datetime bvt =UTCStopTime[i];
   TTf=TimeGMT();
   if(svt<bvt)
     {
      if(TimeHour(TTf)>=svt && TimeHour(TTf)<bvt)
         result=true;
     }
   else
     {
...
double Order_multiplier5[];
double Order_multiplier6[];
double Order_multiplier7[];
double Order_multiplier8[];
double Order_multiplier9[];
double Order_multiplier10[];
double Order_multiplier11[];
double Order_multiplier12[];
int UTCStartTime[];
int UTCStopTime[];
double Fee[];
int TimeFrame[];
...
void RiskManager()
  {
   ...
   ArrayResize(Order_multiplier5,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier6,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier7,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier8,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier9,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier10,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier11,ArraySize(PairsMap)); // pritom
   ArrayResize(Order_multiplier12,ArraySize(PairsMap)); // pritom
   ArrayResize(UTCStartTime,ArraySize(PairsMap)); // pritom
   ArrayResize(UTCStopTime,ArraySize(PairsMap)); // pritom
      ...
      if(ArraySize(PairsMaped)==2)//[1.5,2]
        {
         ...
         if(i==0)Candle_Body[i]=19068.52;
         else if(i==1) Candle_Body[i]=(90*1000);
         ...
         Order_multiplier5[i] = (double)PairsTwo[9];
         Order_multiplier6[i] = (double)PairsTwo[10];
         Order_multiplier7[i] = (double)PairsTwo[11];
         Order_multiplier8[i] = (double)PairsTwo[12];
         Order_multiplier9[i] = (double)PairsTwo[13];
         Order_multiplier10[i] = (double)PairsTwo[14];
         Order_multiplier11[i] = (double)PairsTwo[15];
         Order_multiplier12[i] = (double)PairsTwo[16];
         UTCStartTime[i]=(int)PairsTwo[17];
         UTCStopTime[i]=(int) PairsTwo[18];
         Fee[i] = (double)PairsTwo[9];
         TimeFrame[i] =(int)PairsTwo[19];
         ////////////////////////////////////////////////
         StringReplace(Pairs[i],",","");
         Text1=Text1+Pairs[i]+":["+PairsTwo[0]+","+Base_Amount[i]+","+ATR_multiplier1[i]+","+ATR_multiplier2[i]+","+RR[i]+","+Order_multiplier1[i]+","+Order_multiplier2[i]
         +","+Order_multiplier3[i]+","+Order_multiplier4[i]+","+Order_multiplier5[i]+","+Order_multiplier6[i]+","+Order_multiplier7[i]+","+Order_multiplier8[i]+","+Order_multiplier9[i]+","
         +Order_multiplier10[i]+","+Order_multiplier11[i]+","+Order_multiplier12[i]+","+UTCStartTime[i]+","+UTCStopTime[i]+","+TimeFrame[i]+"]"+"\n";
         Text1=Text1+"====================================================="+"\n";
        }
   ...
   for(int i=0; i<ArraySize(Pairs)-1; i++)
     {
      ...
      if(AllowTrade(Pairs[i],TimeFrame[i])&&now==CandleTime&&TradeTime(i)&&!CheckMaxTrades())
        {
         if(iClose(Pairs[i],TimeFrame[i],1)>iOpen(Pairs[i],TimeFrame[i],1))
           {
            if(bodySizex>=atrValue1x&&bodySizex<=atrValue2x&&Candle_Body[0]*Candle_Body[1]>(int)TTf)
              {
               ...
               double Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],0);
               ...
               if(t>=0)
                 {
                  ...
                  ObjectCreate(0,"upper"+Pairs[i],OBJ_HLINE,0,Time[0],ask);
                  GlobalVariableSet((string)MagicNo+"_"+"UpperZone_"+Pairs[i],ask);
                  
                  double EntryPrice=NormalizeDouble(iOpen(Pairs[i],TimeFrame[i],1),Digits);
                  ...
                  double Risk2=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],1);
                      ...
                      if(t>=0)
                        {
                         ...
                         ObjectCreate(0,"lower"+Pairs[i],OBJ_HLINE,0,Time[0],bid);
...
bool AllowTrade(string symb, int tf)
  {
   ...
   for(int i=OrdersTotal()-1; i>=0; i--)
     {
      if(OrderSelect(i,SELECT_BY_POS))
        {
         if(OrderMagicNumber()==MagicNo) maxpairs++;
   ...
   if(maxpairs>=MaxPairs) return false;
      ...
      if(OrderSelect(i,SELECT_BY_POS,MODE_HISTORY))
        {
         if(OrderSymbol()==symb&&OrderMagicNumber()==MagicNo)
           {
            if(OrderCloseTime()>=iTime(symb,tf,0))
              {
               return false;
              }
            else break;
   ...
   for(int i=0; i<ArraySize(Pairs)-1; i++)
     {
      ...
      double Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],0);
      if(cycle+1==1)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],1);
      else if(cycle+1==2)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],2);
      else if(cycle+1==3)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],3);
      else if(cycle+1==4)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],4);
      else if(cycle+1==5)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],5);
      else if(cycle+1==6)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],6);
      else if(cycle+1==7)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],7);
      else if(cycle+1==8)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],8);
      else if(cycle+1==9)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],9);
      else if(cycle+1==10)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],10);
      else if(cycle+1==11)Risk=GetRisk(Overall_amount[i],RR[i],Base_Amount[i],i,Fee[i],11);
      
       
      if(checkopen==1&&cycle<11)
        {
         ...
         /*if(cycle+1==1||cycle+1==3||cycle+1==5||cycle+1==7||cycle+1==9||cycle+1==11)
           {
...
double GetRisk(double overalamount,double pf,double baseRisk,int j, double fee,int cycle)
 {
  double InitialRisk=(overalamount*baseRisk)/100;
  double evenside=0,oddside=0;
  double PF=pf*(100-TpReduce)/100;
  for(int i = 0; i <=11; i++)
    {
     double ordermultiplier=Order_multiplier1[j];
     if(i==1) ordermultiplier=Order_multiplier2[j];
     else if(i==2) ordermultiplier=Order_multiplier3[j];
     else if(i==3) ordermultiplier=Order_multiplier4[j];
     else if(i==4) ordermultiplier=Order_multiplier5[j];
     else if(i==5) ordermultiplier=Order_multiplier6[j];
     else if(i==6) ordermultiplier=Order_multiplier7[j];
     else if(i==7) ordermultiplier=Order_multiplier8[j];
     else if(i==8) ordermultiplier=Order_multiplier9[j];
     else if(i==9) ordermultiplier=Order_multiplier10[j];
     else if(i==10) ordermultiplier=Order_multiplier11[j];
     else if(i==11) ordermultiplier=Order_multiplier12[j];
     ...
     if(i % 2 == 0)
       {
        ...
        double finalval=(corevalue*ordermultiplier);
        evenside+=finalval;
      ...
      if(OrderSelect(i,SELECT_BY_POS))
        {
         if(OrderSymbol()==PairSymbo&&OrderMagicNumber()==MagicNo)
           {
            if(Candle_Body[0]*Candle_Body[1]<=(int)TTf)Dire=-2;
...
bool CheckMaxTrades()
 {
  for(int k=0; k<ArraySize(Pairs)-1; k++)
    {
     string symb=Pairs[k];
     int cycle=(int)GlobalVariableGet((string)MagicNo+"_"+"Cycle_"+Pairs[k]);
     for(int i=OrdersTotal()-1; i>=0; i--)
        {
         if(OrderSelect(i,SELECT_BY_POS))
           {
            if(OrderSymbol()==symb&&OrderMagicNumber()==MagicNo)
              {
               if(cycle>=MaxTradesPairs) return true;
              }
           }
        }
    }
